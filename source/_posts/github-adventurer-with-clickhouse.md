...
[《ClickHouse 社群指标模型》](clickhouse-metrics-model.md) 一文里提到了 ClickHouse 社群基于自己的软件 ClickHouse 制作社群指标的探索。由于遇到了公开数据集表模式缺列，查询执行内存限制，以及数据库只读模式限制等问题，我在过去一周里试着按照 [ClickHouse 官方博客](https://ghe.clickhouse.tech/#how-this-dataset-is-created)的介绍，搭建起了一个属于自有的基于 ClickHouse 的 GitHub 事件数据库。
...
面对这种形势，一方面是凸显出云中立的技术厂商的价值，以及原厂能够最大限度的提供新版支持和旧版兼容的优势，另一方面也让我想起了之前看过的一篇文章 [Local-first software](https://www.inkandswitch.com/local-first/) 里面提出的“你拥有自己的数据，而不是云厂商”的理念。
...
话说回来，最终我是用了本地的 Metabase 实例 + 配置 Driver 跑起来了整体应用。借助 Metabase 的 BI 能力，探索出了诸如分时最活跃的参与者这样的一些指标。不过我的可视化功底非常差，做出来的图标不好意思见人。如果实在想看，可以打开[这条推文](https://twitter.com/tison1096/status/1557572123387777024)。
