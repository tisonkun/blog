...
对于现有系统来说，实现这样的转型是不容易的，但也并非不可能。例如 Datastax 公司全力投入支持以 Cassandra 为基础的 Astra DB 在云上的应用，HBase 社群也在投入存储上云的开发。对于新系统来说，历史包袱会轻松一些，能够基于现在的情况针对性的设计。但是在这个增量市场逐渐萎缩的环境下，做好现有系统 API 的支持就是一个关键的生产系统采用考量了。例如，[ScyllaDB](https://github.com/scylladb/scylla) 采用了 thread-per-code 的线程模型来试图革新 KV NoSQL 的性能，但是在面向用户的接口上选择兼容 Cassandra 的 API 来帮助存量用户平滑过度。

进一步地，如果在 KV NoSQL 本身的市场打开了局面，作为其他 NoSQL 系统的底座，KV NoSQL 系统可以使用协议层抽象来支持不同场景的数据存储需求。例如 TiKV 是一个支持分布式事务的 KV NoSQL 系统，[Titan](https://github.com/distributedio/titan) 项目通过实现一个 Redis 协议的解析层来支持 Redis Client 对 TiKV 系统的访问。同时，协议层了解 KV 之上具体场景下的数据结构信息，知道用户想要存的是什么数据，可以进一步优化数据访问效率。单纯从系统设计角度说，TiDB 也可以认为是 TiKV 之上加了一个大致支持 MySQL 语义的协议层。
