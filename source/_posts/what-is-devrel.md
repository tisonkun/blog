...
上面描述的是一种相对流畅的开发者体验，其实每一步都有可能出问题。例如，搜索引擎优化做得不好，用户可能根本就找不到这个软件；项目文档缺失或不准确，给到用户软件不可用（破窗）的印象，用户可能根本不会也无法尝试；遇到各类具体问题找不到答案也无人理睬，如果软件源代码不可得，或者用户无法理解代码逻辑，那么他只能寻找其他的替代品。
